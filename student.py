...
class studentClass:
    def __init__(self, root):
        ...
        # Title
        title = Label(self.root, text="Manage Student Details", compound=LEFT,
    ...
    def update(self): 
        ...
        try:
            ...
            if row is None:
                messagebox.showerror("Error", "Select student From List", parent=self.root)
                return  # Exit the function if the student doesn't exist
            ...
            """, (
                ...
                self.var_city.get(),
                self.var_pin.get(),
                self.txt_address.get("1.0", END).strip(),  # Remove any trailing newline
                self.var_roll.get()
            ))
